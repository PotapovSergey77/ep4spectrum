...
**ПЗУ с карты.** Образы 128K, +2A/+3 и Pentagon не зашиваются в ПЛИС, а
...
через отдельный порт-канал. TR-DOS отдельным образом не грузится — он
банк 1 пентагоновского файла. Подробности в [sdcard/README.md](sdcard/README.md).
...
DivMMC перехватывает выборки команд по фиксированным адресам и подменяет собой
`$0000–$3FFF`, а программа, которая рассчитывает видеть там ПЗУ машины,
этого не ждёт.

Самый частый случай — демка, идущая в IM 1 на обработчике `$0038` из
ПЗУ при выбранном 48 BASIC: это ровно то условие, по которому взводится
автомаппер, и ESXDOS впаивается поверх ПЗУ каждый кадр. Поэтому есть
**F10** — выключатель автомаппера, как переключатель на настоящей плате
DivMMC. Загрузить программу через ESXDOS, нажать F10, запустить. Сброс
всегда возвращает DivMMC включённым, так что остаться без пути к ESXDOS
нельзя.

Выключенный DivMMC молчит и на портах: запись в `$E3`, `$E7`, `$EB`
не доходит до него, чтение не возвращает его данные. Это нужно тестам,
которые перебирают все порты подряд, — `ttst48` в тесте 35 пишет `$E3` в
порт `$E3E3`, а с включённым DivMMC это CONMEM и MAPRAM, то есть ESXDOS
поверх ПЗУ, как было бы и с настоящим интерфейсом.

Кроме того, на плате есть два пути к карте, не трогающих карту памяти
вовсе.
...
| F10 | выключить/включить DivMMC: автомаппер и порты `$E3`/`$E7`/`$EB` |
| F11 | сброс |
| F12 | NMI: вход в меню ESXDOS с любого ПЗУ |
...
**F12** переводит всё, чего касается DivMMC, в то состояние, которое у
машины есть при включении питания: условие входов автомаппера, доступность
TR-DOS и владение картой. Это единственное состояние, в котором ESXDOS
заведомо работает, потому что при старте слоты пусты и слотовая
машинерия ему не мешает. Снимается сбросом — F11 возвращает машину на её
собственное ПЗУ.

**F11 с зажатым пробелом** стирает флаги загруженных слотов ПЗУ, и
машина поднимается на DivMMC с ESXDOS уже насовсем, до следующей
загрузки образов.
...
| `LED[2]` | 85 | автомаппер DivMMC выключен (F10) |
| `LED[1]` | 86 | не используется |
...
### romload и pentagon.rom

Два файла из архива стоит назвать отдельно, потому что без них Пентагон
не поднимется:

| файл | куда | что это |
|---|---|---|
| [sdcard/bin/romload](sdcard/bin/romload) | `BIN` на карте | точечная команда, кладёт образ в слот |
| [sdcard/pentagon.rom](sdcard/pentagon.rom) | **корень** карты | сам образ, 64 КБ |

`romload` — обычная точечная команда ESXDOS, поэтому её место в каталоге
`BIN`, рядом с `.browse` и остальными. **Аргументов у неё нет**:
запускается как `.romload`, читает `pentagon.rom` из корня карты и
раскладывает его по слотам в SDRAM. Каждому файлу отводится строка на
экране — имя и полоска из звёздочек, которая доходит до правого края
ровно тогда, когда файл загружен.

`pentagon.rom` — один образ на 64 КБ, четыре банка по 16:
...
Так этот образ устроен у настоящего Пентагона, и наша адресация
повторяет его: два бита выбора — бит 4 порта `$7FFD` и сигнал TR-DOS.
Именно поэтому TR-DOS больше не отдельный слот, а банк 1 этого образа.

Пока слот не помечен заполненным, машина берёт **внутреннее ПЗУ 48K** из
блочной памяти ПЛИС. Поэтому карта без этих файлов всё равно грузится —
и может сказать, чего не хватает. Записать в область ПЗУ из программы
нельзя никак: у `$0000–$3FFF` нет пути записи вовсе, а адрес, куда
ложится байт, задаёт счётчик внутри ПЛИС, а не шина адреса.
### Автозагрузка
...
### Тайминги Pentagon верны

Демка **Across** рисует и картинку, и бордюр счётом тактов процессора:
такое изображение получается правильным только если кадр, строка и
положение прерывания совпадают с настоящей машиной до такта.
...
Три демки, считающие такты, выглядят правильно одновременно, а Tact
Meter совпадает с оригинальной машиной во всех диапазонах, включая
C000-FFFF. Величины заданы таблицами `video.v`, без подстроек:

- **contention начинается на такт раньше первого пикселя растра**: узор
  6,5,4,3,2,1,0,0 стартует на такте 14335, байт 16384 показывается на
  14336;
- **фронт прерывания смещён с момента выборки процессора**, иначе приём
  попадает то в этот такт, то в следующий;
- **растр задержан на девять пикселей**: один — перекос между двумя
  путями вывода, восемь — целая группа;
...
Сигнал строится скандаблером: строка ровно 896 тактов, 31.25 кГц, 624
строки в кадре, кадровый импульс восемь строк, синхронизация раздельная.
